...
public class DataSourceHelper {
  ...
  public void init() {
    try {
      ...
      dbss.executeUpdate("CREATE TABLE IF NOT EXISTS `tb` ( `id` INT AUTO_INCREMENT COMMENT '数据表序号(自增)', `dbid` INT COMMENT '数据库序号', `name` VARCHAR(255) COMMENT '数据表名', `fk` INT DEFAULT -1 COMMENT '外键', PRIMARY KEY (`id`), CONSTRAINT tb_db_id_fk FOREIGN KEY (`dbid`) REFERENCES `db` (`id`)) ENGINE = InnoDB DEFAULT CHARACTER SET = utf8;");
      dbss.executeUpdate("CREATE TABLE IF NOT EXISTS `dt` ( `id` INT AUTO_INCREMENT COMMENT '数据序号(自增)', `tbid` INT COMMENT '数据表序号', `name` VARCHAR(255) COMMENT '数据名', `fk` INT DEFAULT -1 COMMENT '外键', PRIMARY KEY (`id`), CONSTRAINT dt_tb_id_fk FOREIGN KEY (`tbid`) REFERENCES `tb` (`id`)) ENGINE = InnoDB DEFAULT CHARACTER SET = utf8;");
      dbss.close();
      dbsc.close();
